...
const Garantee = (props) => {
  ...
  useEffect(() => {
    ...
    fetch(
      env.siteApi +
        (RxStock == "rx"
          ? "/order/manufacture/list"
          : "/order/stock/adminlist"),
      postOptions
    )
      .then((res) => res.json())
      .then(
  ...
  const sendGarantee = () => {
    ...
    const body = {
      orderNo: content.orderData.rxOrderNo
        ? content.orderData.rxOrderNo
        : content.orderData.stockOrderNo,
    ...
    fetch(env.siteApi + "/panel/order/setGuranteeOrder", postOptions)
      ...
      .then(
        (result) => {
          ...
          {
            RxStock == "stock"
              ? window.open("/print-guaranteeStock/" + OrderNum, "_blank")
              : window.open("/print-guaranteeRx/" + OrderNum, "_blank");
          }
        },
        (error) => {
          setLoading(0);
  ...
  if (!content || !List) return <div>Waiting</div>;
  console.log(Table);
  return (
      ...
      <div className="list-container">
        <div className="lathe-container">
          <div className="container">
            <div className="search-container">
              <StyleInput
                ...
                doAction={(e) =>
                  e.keyCode === 13 ? setOrderID(search) : console.log("common")
                }
                defaultValue={
                  content.orderData && content.orderData.rxOrderNo
                    ? content.orderData.rxOrderNo
                    : content.orderData.stockOrderNo
                }
              />
              ...
              <button
                onClick={() => setPageType("hand")}
                className="search-btn garantee-btn"
              >
                گارانتی المثنی
              </button>
            </div>
                  ...
                  <div className="brand-select-wrapper">
                    ...
                    <StyleSelect
                      ...
                      options={
                        (manufactureList &&
                          (RxStock == "rx"
                            ? manufactureList.material
                            : manufactureList.materialList)) ||
                        []
                      }
                      style={{ width: "100%" }}
                    ...
                    <StyleSelect
                      ...
                      options={
                        (manufactureList &&
                          (RxStock == "rx"
                            ? manufactureList.lenzIndex
                            : manufactureList.lenzIndexList)) ||
                        []
                      }
                      style={{ width: "100%" }}
          ...
          <div className="image-wrapper">
            {RxStock == "stock" && PageType !== "hand" ? (
              <img
                src={
                  env.siteApiUrl +
                  (content.orderData.stockFaktor &&
                  content.orderData.stockFaktor[1] &&
                  content.orderData.stockFaktor[1].productDetail
                    ? content.orderData.stockFaktor[1].productDetail.imageUrl
                    : "")
                }
                alt="product image"
              />
            ) : (
              <img src="/img/brands/ESSENCE.svg" alt="Rx" />
            )}
          </div>
        </div>

        {PageType !== "hand" ? (
          <div className="fake-input product-title">
            <p>
              {content.lData.facoryName +
                "|" +
                content.lData.lenzType +
                "|" +
                content.lData.lenzDesign +
                "|" +
                content.lData.lenzIndex +
                "|" +
                content.lData.material}
            </p>
            <span>نام محصول</span>
          </div>
        ) : (
            ...
            {PageType !== "hand" ? (
              ...
              <>
                <div className="calc-btn">
                  <div
                    onClick={() => setCalc("pos")}
                    className={Calc == "pos" ? "d-btn pos-btn" : "d-btn"}
                  >
                    +
                  </div>
                  <div
                    onClick={() => setCalc("neg")}
                    className={Calc == "neg" ? "d-btn neg-btn" : "d-btn"}
                  >
                    -
                  </div>
                </div>
        ...
        {PageType !== "hand" ? (
          <div className="info-container">
            ...
            <div className="fake-input">
              <p>
                {content.orderData.consumer
                  ? content.orderData.consumer
                  : content.userData.cName}
              </p>
            ...
            <div className="btn-wrapper">
              <button onClick={() => sendGarantee()} className="submit">
                ذخیره و چاپ
              </button>
            </div>
          </div>
