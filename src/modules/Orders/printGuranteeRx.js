...
function PrintGurantee(props) {
  ...
  const pureTitle = (title) => {
    if (title.includes("2/2")) return title.replace("2/2 ", "");
    if (title.includes("2/4")) return title.replace("2/4 ", "");
    return title;
  };
    ...
    <div className="printArea fishPrintArea guranteePrint">
      ...
      <table className="hesabfaMainTable">
        <tbody>
          ...
          <tr>
            <td colSpan={5} className="padding-td">
              {OrderLdetail.brandName &&
                OrderLdetail.brandName.toUpperCase() +
                  " " +
                  OrderLdetail.lenzIndex +
                  " " +
                  OrderLdetail.material}
            </td>
          </tr>
          <tr>
            <td colSpan={5} className="padding-td">
              {orderInfo && orderInfo.orderData.consumer
                ? orderInfo.orderData.consumer
                : "MGM Lens"}
            </td>
          </tr>
        </tbody>
