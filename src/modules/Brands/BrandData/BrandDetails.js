...
function BrandDetails(props) {
  ...
  const updateFactory = (index) => {
    ...
    if (tempArray.length === 1) props.setUserFactory([]);
    else {
  ...
  return (
    <div className="serviceItem">
      <StyleInput
        title={formtrans.title[props.lang] + "(مانند: کداک)"}
      ...
      />
      <StyleInput
        title={formtrans.brandCode[props.lang] + " (مانند: kodak)"}
        direction={props.direction}
      ...
      />
      <StyleInput
        title={formtrans.purchaseContact[props.lang]}
        direction={props.direction}
        defaultValue={content ? content.purchaseContact : ""}
        class={"formInput"}
        action={(e) =>
          props.setBrandChange((prevState) => ({
            ...prevState,
      ...
      />
      <StyleInput
        title={formtrans.discountPerc[props.lang]}
        direction={props.direction}
        defaultValue={content ? content.purchase : ""}
        class={"formInput"}
        action={(e) =>
          props.setBrandChange((prevState) => ({
            ...prevState,
      ...
      <div class="formInput">
        <Autocomplete
          ...
          style={{ width: "100%" }}
          defaultValue={props.DefAccess && props.DefAccess}
          onChange={(e, value) => props.setAccess(value)}
          renderInput={(params) => (
            <TextField {...params} label="دسترسی" variant="outlined" />
          )}
        />
      </div>
      <div className="new-member">
        <StyleSelect
          title={formtrans.factory[props.lang]}
          direction={props.direction}
          ...
          action={(e) =>
            props.userFactory
              ? props.setUserFactory([...props.userFactory, e])
      ...
      <div className="factoryList">
        {props.userFactory &&
          props.userFactory.length &&
          props.userFactory.map((factory, i) => (
            <div
              className="factoryItem"
              key={i}
      ...
      <textarea
        placeholder={formtrans.metaDescription[props.lang]}
      ...
      {manageFacory ? (
        <Modal
          title={formtrans.factoryManage[props.lang]}
          lang={props.lang}
          direction={props.direction}
          setLoader={setLoader}
          addItem={addItem}
          close={setManageFactory}
          token={props.token}
          options={factoryList}
          setFactoryState={props.setFactoryState}
          setFactoryCode={props.setFactoryCode}
          factoryState={props.factoryState}
        />
      ) : (
        <></>
      )}
