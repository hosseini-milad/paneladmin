...
export function jalali_to_gregorian_obj(jy, jm, jd) {
  var sal_a, gy, gm, gd, days;
  jy += 1595;
  days =
    -355668 +
  ...
  var totalPage = orderInfo.size
    ? parseInt(
        parseInt(orderInfo.size) /
          parseInt(filters && filters.pageSize ? filters.pageSize : "10")
      )
    : 0;
...
export function sumPriceNew(priceText, minusPrice) {
  ...
  var minusPrice2 = minusPrice
    ? parseInt(minusPrice.toString().replace(/\D/g, ""))
    : 0;
  var rawPrice = parseInt(priceText.toString().replace(/\D/g, ""));
  //console.log(rawPrice,minusPrice)
  return (rawPrice + minusPrice2)
    .toString()
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",")
    .replace(/^\D+/g, "");
}
export const standardService = (serviceTitle) => {
  ...
  if (serviceTitle.includes("{")) {
    ...
    return serviceParse.map((item, i) => (
      <>
        <span key={i}>{item.title}</span>
        <br />
      </>
    ));
  } else return serviceTitle;
};
export function purePrice(priceText) {
  if (!priceText) return 0;
  var rawPrice = priceText.toString().replaceAll(",", "");
  //console.log(rawPrice,priceText)
  return rawPrice.toString().replace(/^\D+/g, "");
}
...
export const CheckAccess = (token, pageUrl) => {
  ...
  var access =
    token.profile && token.profile.find((item) => item.title === pageUrl);
